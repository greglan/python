...
class G_means:
    ...
    """
    def __init__(self, data, alpha, distance):
        self.data = data
        self.alpha = alpha
    ...
    def run(self):
        ...
        while k_former != self.k:
            k_former = self.k
            k_means = GeneralizedLlyod(self.data, self.k, self.distance)
            k_means.run()

            self.centers = k_means.centers
            self.clusters = k_means.clusters

            for cluster in k_means.clusters:
                if len(cluster) >= 20 and not self.is_gaussian(cluster):
                    self.k += 1
            print("Current cluster size: " + str(self.k))
